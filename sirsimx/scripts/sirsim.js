let sketch = (p) => {
   ...
   * @property {number} defaultValues.detectionSuccessRate
   */
  const defaultValues = {
    popsize: 300,
...
//正規分布による乱数生成メソッド 引数を平均値とし、分散値は900で固定している。    
  function getRandom(ave){
    /*
    * Math.rondom()  :[0,1)の乱数の生成
    * Math.sqrt(x)   :xの平方根を返す
    * Math.log(x)    :xを真数として底eの対数を返す
    * Math.sin(x)    :xの正弦を返す
    * Math.floor(x)  :xの小数点を切り捨てる
    * ボックス・ミュラー法を利用する
    */
    var a = 0; var b = 0;
    while(a === 0) a = Math.random(); //[0,1)を(0,1)の乱数にする
    while(b === 0) b = Math.random();   
    var c = Math.sqrt(-2 * Math.log(a));
    c = c * Math.sin(Math.PI * 2 * b) * 900 + ave;
    if (c < 0) c = 0;
    //console.log("value:",c);
    return Math.floor(c);
    }
  ...
  */
  p.checkForRecovered = () => {
    ballsInfectionTime.forEach(ball => {
      var value = getRandom(recoveryTimeInMillis);
      if (ball.time !== undefined &&
        (Date.now() - ball.time > value)) {
        ball.time = undefined;
  ...
  p.checkForDetection = () => {
    ballsInfectionTime.forEach(function(ball,index){
      if(balls[ball.index].status === status.INFECTIOUS && detectionSuccessRate > Math.random()){
        balls[ball.index].status = status.QUARANTINED;
        ballsInfectionTime.splice(index,1);
      }
    })
  }
