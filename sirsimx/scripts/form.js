...
function changeSimParams(resetToDefault) {
  if (resetToDefault) {
    function updateRangeValues() {
      ...
      $('#dtactval').html(document.getElementById('detectionSuccessRate').value);
    }
    return () => {
  ...
  let prev_psize, prev_rectime, prev_iprob, prev_speed,prev_dtrate;
  return function () {
    if (resetToDefault) {
      sirsim.reset();
    } else {
      ...
      let dtrate = parseFloat(detectionSuccessRate.value);
      ...
      if(dtrate){
        arg.detectionSuccessRate = dtrate;
      }
      if (prev_psize !== psize || prev_iprob !== iprob ||
        prev_rectime !== rectime || prev_speed !== speed || prev_dtrate !== dtrate) {
        sirsim.reset(arg);
      }
    }
  }
...
$(function () {
  ...
  $("#dtmin").html(document.getElementById('detectionSuccessRate').getAttribute('min'));
  $("#dtmax").html(document.getElementById('detectionSuccessRate').getAttribute('max'));
